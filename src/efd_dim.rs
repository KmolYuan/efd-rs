...
/// Coefficient type.
pub type Coeff<D> = na::OMatrix<f64, <D as EfdDim>::CDim, na::Dyn>;
/// Coordinate view used in the conversion method.
pub type CoordView<'a, D> = na::MatrixView<'a, f64, <D as EfdDim>::Dim, na::U1>;
...
impl EfdDim for D2 {
    ...
    fn from_curve_harmonic(
        ...
        let (mut coeffs, center) =
            get_coeff_center::<Self::CDim, { Self::Trans::DIM }>(curve, harmonic, is_open);
        // Angle of starting point
        let theta = {
            let c = na::MatrixView2::from_slice(coeffs.column(0).data.into_slice());
        ...
        for (i, mut c) in coeffs.column_iter_mut().enumerate() {
            ...
            let mut m = na::MatrixViewMut2::from_slice(c.as_mut_slice());
            m.copy_from(&(theta * &m));
        }
...
impl EfdDim for D3 {
    ...
    fn from_curve_harmonic(
        ...
        // Angle of starting point
        let theta = {
            let c = na::MatrixView2x3::from_slice(coeffs.column(0).data.into_slice());
            let dy = 2. * c.row_product().sum();
            let dx = c.map(pow2).column_sum();
            0.5 * dy.atan2(dx[0] - dx[1])
        };
        for (i, mut c) in coeffs.column_iter_mut().enumerate() {
            let theta = na::Rotation2::new((i + 1) as f64 * -theta);
            let mut m = na::MatrixViewMut2x3::from_slice(c.as_mut_slice());
            m.copy_from(&(theta * &m));
        }
        // Normalize coefficients sign
        if harmonic > 1 && coeffs[(0, 0)] * coeffs[(0, 1)] < 0. {
...
fn get_coeff_center<R: na::DimName, const DIM: usize>(
    curve: &[[f64; DIM]],
    harmonic: usize,
    is_open: bool,
) -> (MatrixRxX<R>, [f64; DIM]) {
    ...
    let phi = &t * TAU / zt;
    let mut coeffs = MatrixRxX::<R>::zeros(harmonic);
    for (i, mut c) in coeffs.column_iter_mut().enumerate() {
        let n = i as f64 + 1.;
        let phi = &phi * n;
